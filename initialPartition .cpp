...
bool liesWithinLongitudeRange(Map smallerPartition, Data dataRow)
{
    return dataRow.coordinate.longitude >= smallerPartition.p_id.top_left.longitude && dataRow.coordinate.longitude <= smallerPartition.p_id.top_right.longitude;
}


// Checks if the dataRow lies within the latitude range of the partition
bool liesWithinLatitudeRange(Map smallerPartition, Data dataRow)
{
    return dataRow.coordinate.latitude <= smallerPartition.p_id.top_left.latitude && dataRow.coordinate.latitude >= smallerPartition.p_id.bottom_left.latitude;
}

// Checks if the POI lies within the given partition
bool liesInPartition(Map smallerPartition, Data dataRow)
{
    if(liesWithinLongitudeRange(smallerPartition, dataRow) && liesWithinLatitudeRange(smallerPartition, dataRow))
    {
        return true;
    }
    else
...
void loadPoIs(vector<Data>& dataRow, vector<Map>& smallerPartitions)
{
    ...
    for(i = 0; i < smallerPartitions.size(); i++)
    {
        ...
        {
            if(liesInPartition(smallerPartitions[i], dataRow[j]))
...
void loadPoIs2(vector<Data>& dataRow, vector<Map>& smallerPartitions)
{
    ...
    for(i = 0; i < smallerPartitions.size(); i++)
    {
        ...
        while(low + 1 != high)
        {
            ...
            if(liesWithinLongitudeRange(tempPartition, dataRow[mid]))
            {
                tempPos = mid;
                posOfUtility = 0;
                ...
                {
                    if(liesInPartition(tempPartition, dataRow[tempPos]))
